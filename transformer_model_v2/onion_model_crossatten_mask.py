...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, embedding_mask ,sequence_mask):
        ...
        if embedding_mask is not None:
            value = value.masked_fill(embedding_mask == 0, 0.0)
        ...
        if sequence_mask is not None:
            # scores += attn_mask
            scores = scores.masked_fill(sequence_mask == 0, float(-10000.0))
...
def self_attention(embed_input, multihead_attn):
    return multihead_attn(query=embed_input, key=embed_input, value=embed_input,embedding_mask=None,sequence_mask=None)


def cross_attention(posi, self_attn_output, multihead_attn):
    return multihead_attn(query=posi, key=self_attn_output, value=self_attn_output,embedding_mask=None,sequence_mask=None)###有点问题
...
class Encoder_Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.norm1 = nn.LayerNorm(config.max_rz_len, bias=config.bias)
        # self.attn = CausalSelfAttention(config)
        self.attn = MultiHeadAttention(config.max_rz_len,config.n_head)
        self.norm2 = nn.LayerNorm(config.max_rz_len, bias=config.bias)
        self.mlp = MLP(config)
        self.dropout = nn.Dropout(0.1)

    def forward(self, x, embedding_mask=None,sequence_mask=None):
        attn_output, _ = self.attn(x, x, x,embedding_mask,sequence_mask)
        attn_output = self.dropout(attn_output)
        out1 = self.norm1(x + attn_output)
        out1 = self.mlp(out1)
        # Feed forward network
        ffn_output = self.dropout(out1)
...
class vec_compress(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.dropout = nn.Dropout(config.dropout)
        self.c3= nn.Conv1d(in_channels=config.max_rz_len, out_channels=config.max_rz_len, kernel_size=3)
...
class Onion(nn.Module):
    ...
    def forward(self, input, regi, posi,info, embedding_mask,sequence_mask):
        ...
        # sequence_lengh = info[1]*info[2]
        for block in self.block_stack:
            vec = block(vec,embedding_mask,sequence_mask)
        ...
        vec_out =self.vec_compress([vec[:, :info[i][0], :] for i in range(len(info))][0])
        # vec_out =self.vec_compress(vec[:,:info[:,0],:])* regi[:, 0, :].unsqueeze(1)
