...
def pad_arrays_to_rz(arrays, max_r, max_z):
    result = []
    for arr in arrays:
...
class OnionDataset(Dataset):
    def __init__(self, pth_, max_input_len=100, max_r=100, max_z=100):
        ...
        outputs_list = [y[str(i)][:].reshape(int(self.info_list[i][1]), int(self.info_list[i][2])) for i in range(len(y))]  # 收集输出数据
        ...
        for i, r, z in set(self.info_list):
            ...
            posi_list[i] = posi_list[i].reshape(posi_list[i].shape[0], r, z)
            regi_list[i] = regi_list[i].reshape(r, z)

        self.max_input_len = max_input_len
        self.padded_input, self.input_len_org = pad_arrays_to_length(inputs_list, max_input_len)
        self.padded_output = [item.flatten() for item in pad_arrays_to_rz(outputs_list, max_r, max_z)]
        self.padded_regi = pad_arrays_to_rz(regi_list, max_r, max_z)
        self.padded_posi = pad_arrays_to_rz(posi_list, max_r, max_z)
    ...
    def __getitem__(self, idx):
        ...
        posi = self.padded_posi[regi_pos_idx]
...
dataset = OnionDataset("../data_Phantom/phantomdata/mini_1_train_database.h5")
